...
struct ClientFeatureOverrides: Hashable {
  ...
  static func fromList(_ pigeonVar_list: [Any?]) -> ClientFeatureOverrides? {
    ...
    let interceptorOptions: String? = nilOrValue(pigeonVar_list[8])
    let claimCreationTimeoutDurationInMins: Int64? = nilOrValue(pigeonVar_list[9])
    let sessionNoActivityTimeoutDurationInMins: Int64? = nilOrValue(pigeonVar_list[10])
    let aiProviderNoActivityTimeoutDurationInSecs: Int64? = nilOrValue(pigeonVar_list[11])
    let pageLoadedCompletedDebounceTimeoutMs: Int64? = nilOrValue(pigeonVar_list[12])
    let potentialLoginTimeoutS: Int64? = nilOrValue(pigeonVar_list[13])
    let screenshotCaptureIntervalSeconds: Int64? = nilOrValue(pigeonVar_list[14])
