  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        ...
        codec:nullGetMessagesCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(createSessionAppId:providerId:sessionId:completion:)], @"ReclaimHostOverridesApi api (%@) doesn't respond to @selector(createSessionAppId:providerId:sessionId:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray<id> *args = message;
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        ...
        codec:nullGetMessagesCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(logSessionAppId:providerId:sessionId:logType:completion:)], @"ReclaimHostOverridesApi api (%@) doesn't respond to @selector(logSessionAppId:providerId:sessionId:logType:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray<id> *args = message;
        NSString *arg_appId = GetNullableObjectAtIndex(args, 0);
        NSString *arg_providerId = GetNullableObjectAtIndex(args, 1);
